...
struct v {
  ...
  v operator!() { v&t=*this; return t*(1/sqrt(t%t)); }
...
// Similarly, we declare a white constant vector as well.
  W(1,1,1);
...
f D(v p, v& n, f& M) {
  ...
  for (b=B;*b;++b) {
    ...
    if (!o) { 
      ...
      // arc's center.
      //
      f r=.5*(a&1)+1, t=atan2(p.y-y*.5,p.x-x/2),
    ...
    // Now run the distance query on this primitive
    Q(p,n,C,D,M);
  ...
  n=v(0,1); Q(p,p-n*(p%n+.9),C,D,M,1);
  n=v(0,0,1); Q(p,p-n*(p%n+.9),C,D,M,1);
...
v R(v o, v d, f z) {
  ...
  // Scalar variables:
  //
  //   u: distance along the ray at current point; also, dummy var for material
  ...
  while (u<50)
    ...
    if (fabs(l=D(o+d*(u+=l),n,m))<.001) {
      ...
      p = v(m,m,1)*(U(n%L)/3+.65)*a;

      if (z && m!=1) { p = p*.7 + R(o+n*.1,n,0)*.3; }
